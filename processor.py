...
"""
import os, subprocess, json, struct, math, shutil, sys, time
...
def _get_cookies_arg():
    """Return --cookies flag if YOUTUBE_COOKIES env var is set, else empty string."""
...
def try_download(url, raw_path, timeout=360):
    safe = "--no-playlist --no-check-certificates --socket-timeout 30 --age-limit 99"
    ...
    strategies = [
        f'yt-dlp {safe} {ck} --extractor-args "youtube:player_client=ios" {fmt} {out}',
        f'yt-dlp {safe} {ck} --extractor-args "youtube:player_client=mweb" {fmt} {out}',
        f'yt-dlp {safe} {ck} --extractor-args "youtube:player_client=ios,mweb,android,tv_embedded" {fmt} {out}',
        f'yt-dlp {safe} {ck} --extractor-args "youtube:player_client=tv_embedded,formats=missing_pot" {fmt} {out}',
        f'yt-dlp {safe} {ck} --extractor-args "youtube:player_client=android" {fmt} {out}',
        f'yt-dlp {safe} {ck} --extractor-args "youtube:player_client=web_creator" {fmt} {out}',
        f'yt-dlp {safe} {ck} --extractor-args "youtube:player_client=web_embedded" {fmt} {out}',
        f'yt-dlp --no-playlist --no-check-certificates --age-limit 99 --socket-timeout 60 {ck} --merge-output-format mp4 -f best {out}',
    ]
    last_err = ""
...
def classify_download_error(err_text):
    ...
    if "age" in err and ("restrict" in err or "limit" in err or "mature" in err):
        return "Vídeo com restrição de idade. Tente outro vídeo público."
    if "sign in" in err or "login" in err or "confirm" in err:
        return (
            "O YouTube bloqueou temporariamente o download do nosso servidor (proteção anti-bot). "
        ...
        if len(peaks) < clips:
            ...
            for i in range(clips):
                ...
                if t + clip_dur / 2 <= vid_dur:
                    if all(abs(t - p) >= clip_dur for p in peaks):
                        peaks.append(t)
