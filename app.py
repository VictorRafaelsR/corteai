...
# Allow CORS from all origins (fixes "Erro de conexão" on mobile/cross-origin)
...
def process():
    ...
    # Se não tem URL mas tem player, busca no YouTube automaticamente
    if not url and player:
        url = f"ytsearch1:{player} highlights"
    elif not url:
        return jsonify({"error": "Cole um link do YouTube ou preencha o campo de pesquisa."}), 400

    # Valida URL (aceita youtube.com, youtu.be, ou busca ytsearch)
    is_yt = ("youtube.com" in url or "youtu.be" in url or url.startswith("ytsearch"))
    if not is_yt:
        return jsonify({"error": "Link inválido. Use um link do YouTube (youtube.com ou youtu.be)."}), 400
